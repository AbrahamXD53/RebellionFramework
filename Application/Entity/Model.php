...
class Model
{
    ...
    protected static $protectedFields = array();
    ...
    protected function hasMany($class, $foreign = null, $altname = null)
    {
        if (!$foreign) {
        ...
        $params = ['id' => $this->id];
        $result = $class::runQuery($sql, $params, false, true);
        if ($result) {
            if (!empty($result)) {
                if ($altname) {
                    $this->{$altname} = $result;
                    ...
                    $this->{get_called_class()::$tableNames[$class]} = $result;
                    return $this->{get_called_class()::$tableNames[$class]};
                }
            }
        }
        return $result;
    ...
    protected function hasOne($class, $foreign = null, $altname = null)
    {
        ...
        if ($result) {
            if (!empty($result)) {
                if ($altname) {
                    $this->{$altname} = $result[0];
                    return $this->{$altname};
                } else {
                    $this->{get_called_class()::$tableNames[$class]} = $result[0];
                    return $this->{get_called_class()::$tableNames[$class]};
                }
            }
        }
    ...
    {
        if (!isset(get_called_class()::$columnNames[get_called_class()])) {
            get_called_class()::$columnNames[get_called_class()] = array();
            $sql = Finder::info(get_called_class()::$tableNames[get_called_class()])::getSql();
            $stmt = get_called_class()::$connection->pdo->query($sql);
    ...
    public static function find($id)
    {
        ...
        $stmt = get_called_class()::$connection->pdo->prepare($sql);
    ...
    public static function runQuery($finder, $params = null, $isOperation = false, $forceCast = false)
    {
        ...
        {
            if ($isOperation) {
                get_called_class()::$connection->pdo->beginTransaction();
            }
            $stmt = get_called_class()::$connection->pdo->prepare($query);
            if ($params) {
                $stmt->execute($params);
            } else {
                $stmt->execute();
            ...
            if ($isOperation) {
                if ($params) {
                    ...
                    $result = get_called_class()::$connection->pdo->lastInsertId();
                }

                get_called_class()::$connection->pdo->commit();
            }
        } catch (PDOException $e) {
            error_log(__METHOD__ . ':' . __LINE__ . ':' . $e->getMessage());
            if ($isOperation) {
                get_called_class()::$connection->pdo->rollBack();
            }
            return null;
        } catch (Throwable $e) {
            error_log(__METHOD__ . ':' . __LINE__ . ':' . $e->getMessage());
